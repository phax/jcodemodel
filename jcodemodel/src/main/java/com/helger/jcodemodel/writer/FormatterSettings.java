...
public class FormatterSettings
{
  ...
  public static final Consumer <FormatterSettings> CONF_PHELGER = settings -> {
    settings.indent.useSpaces (2).tabSize (2);
    settings.wrap.method.args.condition (EListWrapStrategy.REQUIRED);
    settings.wrap.method.bracket.condition (EWordWrapStrategy.ALWAYS);
  };
  ...
  public static final Consumer <FormatterSettings> CONF_GLELOUET = settings -> {
    settings.indent.useTabs (1).tabSize (2);
    settings.wrap.catchClause.types.condition (EListWrapStrategy.BINARY).indent (2);
    settings.wrap.forLoop.init.condition (EListWrapStrategy.BINARY).indent (2);
    settings.wrap.method.args.condition (EListWrapStrategy.BINARY).indent (2);
    settings.wrap.method.bracket.condition (EWordWrapStrategy.NEVER);
    settings.wrap.method.params.condition (EListWrapStrategy.BINARY).indent (2);
    settings.wrap.variables.block.condition (EListWrapStrategy.BINARY).indent (2);
    settings.wrap.variables.field.condition (EListWrapStrategy.BINARY).indent (2);
  };
