...
JCM allows to define and use code generators to call a plugin by maven, that writes code generated by the generators.
...
This part focus on how to add a new generator in this project, generic enough to be used by other projects. 
...
I assume you want to also use your generator in that project. In that case it's a bit more complex.

First, you need to have your project be a multi-module project, that means the root module must be a pom package. 

Then you need to decide where you will put the generator ; the maven module the generator will be put under is the *base* module, while the main module of your project is the *root* module.
...
1. add the dependency to *commons*
2. add the dependency to *generator* with `<optional>true</optional>`. This will make sure the modules are sequenced correctly, even though *generator* is not embedded in *generated*
3. configure plugins to import the generated dir, if that's the one you use, and delete it upon clean :
...
In my case I add the plugin call in a *generate* profile, so I also add the `-P generate`, this way the plugin is only applied manually. The clean plugin is also put in that profile to avoid losing the classes when a `mvn clean install` is used.  